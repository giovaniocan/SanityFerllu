...
import { fromEvent, timer, EMPTY, of, merge, map as map$1, startWith as startWith$1, catchError as catchError$1, from, concat, firstValueFrom, filter, BehaviorSubject, defer, mergeMap, throwError, share, partition, take, throttleTime, asyncScheduler, tap as tap$1, debounce } from "rxjs";
...
const useRoleRequestsStatus = () => {
  const client = useClient({ apiVersion: "2024-07-01" }), projectId = useProjectId(), checkRoleRequests = useMemo(() => !client || !projectId ? of({ loading: !1, error: !1, status: "none" }) : from(
    client.request({
    ...
    map((requests) => {
      if (requests && requests.length) {
        ...
        if (declinedRequest && isAfter(addWeeks(new Date(declinedRequest.createdAt), 2), /* @__PURE__ */ new Date()))
          return { loading: !1, error: !1, status: "declined" };
        if (projectRequests.find(
          (request) => request.status === "pending" && isAfter(addWeeks(new Date(request.createdAt), 2), /* @__PURE__ */ new Date())
        ))
          return { loading: !1, error: !1, status: "pending" };
        if (projectRequests.find(
          (request) => request.status === "pending" && isBefore(addWeeks(new Date(request.createdAt), 2), /* @__PURE__ */ new Date())
        ))
          return { loading: !1, error: !1, status: "expired" };
      }
      return { loading: !1, error: !1, status: "none" };
    }),
    catchError((err) => (console.error("Failed to fetch access requests", err), of({ loading: !1, error: !0, status: void 0 }))),
    startWith({ loading: !0, error: !1, status: void 0 })
    // Start with loading state
  ), [client, projectId]), { loading, error, status } = useObservable(checkRoleRequests, {
    loading: !0,
    error: !1,
    status: void 0
  });
...
function PermissionCheckBanner({ granted, requiredPermission }) {
  ...
  ), currentUserRoles = currentUser?.roles || [], isOnlyViewer = currentUserRoles.length === 1 && currentUserRoles[0].name === "viewer", [showRequestPermissionDialog, setShowRequestPermissionDialog] = useState(!1), listFormat = useListFormat({ style: "short" }), { t } = useTranslation(structureLocaleNamespace), telemetry = useTelemetry();
  if (granted) return null;
  const roleTitles = currentUserRoles.map((role) => role.title), roles = listFormat.formatToParts(roleTitles).map(
...
const preventDefault = (ev) => ev.preventDefault(), FormView = forwardRef(function(props, ref) {
  ...
  return /* @__PURE__ */ jsx(
    Container,
    {
      ...
      sizing: "border",
      width: 1,
      children: /* @__PURE__ */ jsx(PresenceOverlay, { margins, children: /* @__PURE__ */ jsx(Box, { as: "form", onSubmit: preventDefault, ref: setRef, children: connectionState === "connecting" || !ready ? /* @__PURE__ */ jsx(Delay, { ms: 300, children: /* @__PURE__ */ jsxs(Flex, { align: "center", direction: "column", height: "fill", justify: "center", children: [
